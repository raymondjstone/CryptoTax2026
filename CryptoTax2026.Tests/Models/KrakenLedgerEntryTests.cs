...
public class KrakenLedgerEntryTests
{
    ...
    [Theory]
    [InlineData("ETH.F", "ETH")]
    [InlineData("DOT.F", "DOT")]
    [InlineData("SOL.F", "SOL")]
    [InlineData("ADA.F", "ADA")]
    [InlineData("TIA.F", "TIA")]
    public void NormaliseAssetName_RemovesFlexStakingSuffix(string krakenName, string expected)
    {
        Assert.Equal(expected, KrakenLedgerEntry.NormaliseAssetName(krakenName));
    }

    [Theory]
    [InlineData("DOT.B", "DOT")]
    [InlineData("KSM.B", "KSM")]
    public void NormaliseAssetName_RemovesBondedStakingSuffix(string krakenName, string expected)
    {
        Assert.Equal(expected, KrakenLedgerEntry.NormaliseAssetName(krakenName));
    }

    [Theory]
    [InlineData("ETH.M", "ETH")]
    [InlineData("KSM.P", "KSM")]
    public void NormaliseAssetName_RemovesMarginAndParachainSuffix(string krakenName, string expected)
    {
        Assert.Equal(expected, KrakenLedgerEntry.NormaliseAssetName(krakenName));
    }
    ...
    // ========== Nested Suffix Normalisation ==========

    [Theory]
    [InlineData("XETH.F", "ETH")]  // Kraken prefix + flex suffix
    [InlineData("XXBT.S", "BTC")]  // Legacy BTC prefix + staked suffix
    [InlineData("XXDG.F", "DOGE")] // Legacy DOGE prefix + flex suffix
    public void NormaliseAssetName_HandlesNestedPrefixAndSuffix(string krakenName, string expected)
    {
        Assert.Equal(expected, KrakenLedgerEntry.NormaliseAssetName(krakenName));
    }
}
